...
i ++) {
for(int sphereno = 0;
sphereno < spheres.data.length();
sphereno ++) {
newhitinfo = RaySphere(ray, spheres.data[sphereno].xyz, spheres.data[sphereno].w, sphere_materials.materials[sphereno]);
...
vec3 viewparams = vec3(planeHeight * aspect_ratio, planeHeight, - camera_data.Near);
...
imageStore(output_render, invocID, total_colour / num_rays * 2);
