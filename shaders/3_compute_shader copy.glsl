...
vec4 Trace(Ray ray) {
    ...
    for(int i = 0; i < MAX_BOUNCES; i++) {
        for(int sphereno = 0; sphereno < spheres.length(); sphereno++) {
            newhitinfo = RaySphere(ray, spheres.data[sphereno].xyz, spheres.data[sphereno].w, gen_material_from_sphereno(sphereno));
            if(!hitinfo.didHit) hitinfo = newhitinfo;
            else if(newhitinfo.dst < hitinfo.dst) hitinfo = newhitinfo;
        }
        ray.origin = hitinfo.pos;
        ray.direction = randDir(hitinfo.normal.xy, hitinfo.normal.xz, hitinfo.normal.yz);
        vec3 emittedLight = hitinfo.material.emission_colour * hitinfo.material.emission_strength;
...
void main() {
    ...
    imageStore(output_render, invocID, Trace(ray))
    ...
    //     HitInfo thehit = RaySphere(ray, sphere.xyz, sphere.w, gen_material(sphereMaterials.colour[i], sphereMaterials.emission_colour[i], sphereMaterials.emission_strength[i]));
